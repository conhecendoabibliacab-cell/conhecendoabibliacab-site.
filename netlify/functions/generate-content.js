...
function buildGeminiEndpoint(key, model) {
  const apiVersion = process.env.GEMINI_API_VERSION || 'v1';
  const effectiveModel = model || process.env.GEMINI_MODEL || 'gemini-2.5-pro';
  return `https://generativelanguage.googleapis.com/${apiVersion}/models/${effectiveModel}:generateContent?key=${key}`;
}

async function tryModel(kind, key, model) {
  const endpoint = buildGeminiEndpoint(key, model);
  const body = { contents: [{ parts: [{ text: buildPrompt(kind) }] }] };
  const res = await fetch(endpoint, {
    method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body)
  });
  if (!res.ok) {
    const errText = await res.text();
    throw new Error(`Gemini API error (${model || 'default'}): ${res.status} ${errText}`);
  }
  const data = await res.json();
  const candidate = data.candidates && data.candidates[0];
  const parts = candidate && candidate.content && candidate.content.parts;
  const text = parts && parts[0] && (parts[0].text || (parts[0].inline_data && parts[0].inline_data.data)) || '';
  return { text: text || 'Sem conteúdo retornado.', modelUsed: model || process.env.GEMINI_MODEL || 'gemini-2.5-pro' };
}

async function generateWithGemini(kind) {
  const key = process.env.GOOGLE_API_KEY;
  if (!key) {
    return { text: `Placeholder (${kind}): configure GOOGLE_API_KEY para geração real.`, modelUsed: '—' };
  }
  const desired = process.env.GEMINI_MODEL || null;
  const candidates = [desired, 'gemini-2.5-pro', 'gemini-1.5-pro', 'gemini-1.5-flash'].filter(Boolean);
  let lastErr = null;
  for (const m of candidates) {
    try { return await tryModel(kind, key, m); } catch (e) { lastErr = e; }
  }
...
exports.handler = async (event) => {
  ...
  try {
    ...
    return { statusCode: 200, headers, body: JSON.stringify({ kind, text: result.text, model: result.modelUsed }) };
  } catch (err) {
